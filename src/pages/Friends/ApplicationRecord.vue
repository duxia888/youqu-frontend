...
const onTabChange = async (name: string) => {
  ...
  if (name === "2") {
    status.value.myApplyFriends = (await myAxios.get("/friends/getMyRecords")).data
  }
}
const agree = async (id: number, fid: number) => {
  const agree = await (await myAxios.post(`/friends/agree/${id}`)).data
  if (agree) {
...
const canceledApply = async (id: number) => {
  const canceledApply = (await myAxios.post(`/friends/canceledApply/${id}`)).data
  if (canceledApply) {
    ...
    <div class="friend-group">
      <ul class="friend-items" v-if="active==='1'">
        ...
        <li v-else v-for="friend in status.applyFriends" :key="friend.id"
          ...
          <div class="avatar-container">
            <img @click="showUser(friend.applyUser.id)" :src="friend.applyUser.avatarUrl||defaultPicture"
                 class="avatar">
          </div>
      ...
      <ul class="friend-items" v-else>
        ...
        <li v-else v-for="friend in status.myApplyFriends" :key="friend.id"
            class="friend-item">
          <div class="avatar-container">
            <img @click="showUser(friend.applyUserid)" :src="friend.applyUser.avatarUrl||defaultPicture"
                 class="avatar">
          </div>
          <div @click="showUser(friend.applyUser.id)" class="friend-info">
            <div class="friend-apply">
