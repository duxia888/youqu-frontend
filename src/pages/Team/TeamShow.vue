...
import {defaultPicture, jsonParseTag} from "../../components/userCommon";
import {Dialog, Toast} from "vant";
import {TeamListType} from "../../models/team";
...
onMounted(async () => {
  await getCurrent()
  const users: any[] = (await myAxios.get(`/team/${teamId}`)).data
  console.log(users)
  if (users) {
    team.value = users
...
<template>
  <van-sticky :offset-top="40">
    <van-notice-bar v-if="team.description"
                    left-icon="volume-o"
                    color="#1989fa" background="#ecf9ff"
                    :text="'队伍公告：'+team.description"
    />
  </van-sticky>
